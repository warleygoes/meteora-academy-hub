...
Deno.serve(async (req) => {
  ...
  if (req.method === "DELETE") {
    try {
      ...
      await supabaseAdmin.from('lesson_ratings').delete().eq('user_id', user_id);
      await supabaseAdmin.from('lesson_comments').delete().eq('user_id', user_id);
      await supabaseAdmin.from('user_lesson_access').delete().eq('user_id', user_id);
      await supabaseAdmin.from('course_enrollments').delete().eq('user_id', user_id);
      await supabaseAdmin.from('community_likes').delete().eq('user_id', user_id);
