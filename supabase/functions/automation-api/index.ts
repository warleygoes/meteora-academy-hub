...
async function handleListUsers(
  ...
  let query = supabase
    .from("profiles")
    .select("id, user_id, email, display_name, phone, country, company_name, role_type, client_count, network_type, status, approved, created_at, updated_at", { count: "exact" })
...
async function handleFindUser(
  ...
  if (!params.email) return json({ error: "email is required" }, 400);
  ...
  return json({
    user: {
      ...
      cheapest_plan_usd: profile.cheapest_plan_usd,
      main_problems: profile.main_problems,
      main_desires: profile.main_desires,
      bio: profile.bio,
