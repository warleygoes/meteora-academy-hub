...
const CoursePage: React.FC = () => {
  ...
  const [accessiblePrivateLessons, setAccessiblePrivateLessons] = useState<
    Set<string>
  >(new Set())
  ...
  const fetchData = useCallback(async () => {
    ...
    setAccessiblePrivateLessons(
      new Set((accessData || []).map((a) => a.lesson_id)),
    )
    ...
    const builtModules: Module[] = (modulesData || []).map((m) => ({
      ...
      lessons: (lessonsData || [])
        ...
        .map((l) => ({
          ...
          contents: ((contentsData || []) as LessonContent[]).filter(
            (c) => c.lesson_id === l.id,
          ),
        })),
    }))
    ...
    if (resumeRef.current) {
      ...
      allL.forEach((l, idx) => {
        if (completed.has(l.id)) lastCompletedIdx = idx
      })
      const nextIdx = lastCompletedIdx + 1
      const targetLesson =
        nextIdx < allL.length ? allL[nextIdx] : allL[allL.length - 1]
      if (targetLesson && canAccess(targetLesson)) {
        setActiveLessonId(targetLesson.id)
        const parentMod = builtModules.find((m) =>
        ...
        if (nextAccessible) {
          ...
          const parentMod = builtModules.find((m) =>
            m.lessons.some((l) => l.id === nextAccessible.id),
          )
          if (parentMod) setExpandedModules(new Set([parentMod.id]))
        }
      }
  ...
  const renderContent = (content: LessonContent) => {
    switch (content.type) {
      case "video":
        return content.content ? <VideoPlayer url={content.content} /> : null
      case "text":
        return (
      ...
      <ScrollArea className="flex-1 overflow-y-auto">
        {activeLesson ? (
          ...
          >
            ...
            {/* Rating */}
            {courseId && (
              <LessonRating lessonId={activeLesson.id} courseId={courseId} />
            )}
                ...
                return (
                  <>
                    <Button
                      ...
                      disabled={idx <= 0}
                      onClick={() =>
                        idx > 0 && selectLesson(allLessons[idx - 1].id)
                      }
                    >
                      ← {t("back") || "Anterior"}
                    </Button>
              ...
              <div key={mod.id}>
                ...
                >
                  ...
                  mod.lessons.every((l) => completedLessons.has(l.id)) ? (
                    ...
                    >
                      {
                        mod.lessons.filter((l) => completedLessons.has(l.id))
                          .length
                      }
                      /{mod.lessons.length}
                    </Badge>
                  ...
                  <div className="ml-4 space-y-0.5">
                    {mod.lessons.map((lesson, li) => {
                      ...
                      return (
                        <button
                          key={lesson.id}
                          onClick={() => hasAccess && selectLesson(lesson.id)}
                          className={`w-full flex items-center gap-2 px-3 py-1.5 rounded-md text-left text-sm transition-colors ${
                            !hasAccess
