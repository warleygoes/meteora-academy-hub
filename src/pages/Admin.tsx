...
import {
  Users, BookOpen, DollarSign, TrendingUp, Shield, Plus, Trash2,
  CheckCircle2, XCircle, Search, ChevronDown, Eye, Ban, UserCheck,
  Clock, Globe, Building2, Phone, Wifi
} from 'lucide-react';
...
const AdminPage: React.FC = () => {
  ...
  const addAdmin = async () => {
    ...
    const { error } = await supabase
      .from('user_roles')
      .insert({ user_id: profiles[0].user_id, role: 'admin' as any });
    if (error) {
      toast({ title: error.code === '23505' ? 'Ya es administrador.' : error.message, variant: 'destructive' });
    } else {
  ...
  return (
    <div className="px-6 md:px-12 py-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-display font-bold">{t('adminPanel')}</h1>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="mb-6">
          <TabsTrigger value="pending" className="gap-2">
            ...
            {stats.pending > 0 && (
              <Badge variant="destructive" className="ml-1 text-xs h-5 min-w-[20px] px-1.5">{stats.pending}</Badge>
            )}
          </TabsTrigger>
                    ...
                    <tr key={user.id} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                      ...
                      <td className="py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          {user.approved ? (
                            <Button variant="ghost" size="sm" onClick={() => suspendUser(user.user_id)} className="text-destructive hover:text-destructive">
                              <Ban className="w-4 h-4" />
                            </Button>
                          ) : (
                            <Button variant="ghost" size="sm" onClick={() => approveUser(user.user_id)} className="text-green-500 hover:text-green-500">
