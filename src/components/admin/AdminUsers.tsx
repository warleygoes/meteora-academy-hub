...
const AdminUsers: React.FC<AdminUsersProps> = ({ stats, onStatsUpdate }) => {
  ...
  const approveAllFromAll = async () => {
    const unapproved = allUsers.filter(u => u.status !== 'approved');
    const userIds = unapproved.map(u => u.user_id);
    if (userIds.length === 0) return;
    setPendingUsers(prev => prev.filter(u => !userIds.includes(u.user_id)));
    setRejectedUsers(prev => prev.filter(u => !userIds.includes(u.user_id)));
    setAllUsers(prev => prev.map(u => userIds.includes(u.user_id) ? { ...u, approved: true, status: 'approved' } : u));
    const { error } = await supabase.from('profiles').update({ approved: true, status: 'approved' }).in('user_id', userIds);
    if (error) { toast({ title: error.message, variant: 'destructive' }); fetchPendingUsers(); fetchRejectedUsers(); fetchApprovedUsers(); fetchAllUsers(); }
    else { toast({ title: `${userIds.length} usuarios aprobados` }); fetchPendingUsers(); fetchRejectedUsers(); fetchApprovedUsers(); fetchAllUsers(); }
    setShowApproveAllFromAllConfirm(false);
  };
  ...
  return (
    <div>
        ...
        <TabsContent value="all">
          <div className="flex gap-3 mb-6">
            ...
            {allUsers.some(u => u.status !== 'approved') && (
              <Button size="sm" className="gap-2 bg-green-600 hover:bg-green-700 text-white ml-auto" onClick={() => setShowApproveAllFromAllConfirm(true)}>
                <CheckCircle2 className="w-4 h-4" /> Aprobar Todos ({allUsers.filter(u => u.status !== 'approved').length})
              </Button>
            )}
          </div>
      ...
      {/* Approve All from All Users Confirmation */}
      <AlertDialog open={showApproveAllFromAllConfirm} onOpenChange={setShowApproveAllFromAllConfirm}>
        <AlertDialogContent className="bg-card border-border">
          <AlertDialogHeader>
            <AlertDialogTitle>¿Aprobar todos los usuarios no aprobados?</AlertDialogTitle>
            <AlertDialogDescription>
              Se aprobarán {allUsers.filter(u => u.status !== 'approved').length} usuario(s). Todos podrán acceder a la plataforma inmediatamente.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={approveAllFromAll} className="bg-green-600 text-white hover:bg-green-700">
              <CheckCircle2 className="w-4 h-4 mr-2" /> Aprobar Todos
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
