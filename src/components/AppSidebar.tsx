...
export const AppSidebar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchLinks = async () => {
      ...
      const [{ data: linkPkgs }, { data: linkProds }] = await Promise.all([
        supabase
          .from("menu_link_packages")
          .select("menu_link_id, package_id")
          .in("menu_link_id", linkIds),
        supabase
          .from("menu_link_products")
          .select("menu_link_id, product_id")
          .in("menu_link_id", linkIds),
      ])
      ...
      const visible = links.filter((l) => {
        ...
        return (
          reqPkgs.some((id) => userPkgIds.has(id)) ||
          reqProds.some((id) => userProdIds.has(id))
        )
      })
  ...
  const navItems = [
    { to: "/app", icon: Home, label: t("home") },
    {
      to: "/app/courses",
      icon: BookOpen,
      label: t("myLibrary") || "Minha Biblioteca",
    },
    ...
    { to: "/app/meetings", icon: Video, label: t("liveMeetings") },
    {
      to: "/app/diagnostics",
      icon: Target,
      label: t("myDiagnostics") || "Mis Diagnósticos",
    },
      ...
      {!collapsed && (
        <div className="px-4 mb-4">
          <form
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-lg bg-sidebar-accent text-muted-foreground text-sm focus-within:ring-1 focus-within:ring-primary/30 transition-all"
          >
        ...
        {customLinks.map((link) => {
          ...
          if (link.open_mode === "embed") {
            ...
            return (
              ...
              >
                ...
                {!collapsed && (
                  <span>{link.translatedTitle || link.title}</span>
                )}
              </NavLink>
            )
          }
          return (
            <a
              ...
              rel={
                link.open_mode === "new_tab" ? "noopener noreferrer" : undefined
              }
              onClick={() => setMobileOpen(false)}
              ...
              {!collapsed && (
                <>
                  <span className="flex-1">
                    {link.translatedTitle || link.title}
                  </span>
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-foreground truncate">
                {user?.user_metadata?.display_name || "Aluno"}
              </p>
  ...
  return (
    <>
      ...
      {/* Mobile sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-40 md:hidden transform transition-transform ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        {sidebar}
      </div>
